...
public class StoreGlobalErrorHandler {
	...
	public ExceptionMessage handlerIllegalArgumentException(
			IllegalArgumentException ex, WebRequest webRequest) {
		return buildExceptionMessage(ex, HttpStatus.NOT_IMPLEMENTED, webRequest, LogStatus.MESSAGE_ONLY);
	}



	private ExceptionMessage buildExceptionMessage(IllegalArgumentException ex, HttpStatus status,
			WebRequest webRequest, LogStatus logStatus) {
		String message = ex.toString();
		String statusReason = status.getReasonPhrase();
		int statusCode = status.value();
